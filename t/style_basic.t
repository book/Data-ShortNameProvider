...
for my $style (qw( Basic +Data::ShortNameProvider::Style::Basic )) {
    ...
    is( $sn, "dbit${v}_${ts}__$n", "'$sn' is a short name for '$n'" );
    ...
    is( $np->style_class->new($hash)->generate_new_name('bar'),
        "dbit${v}_${ts}__bar", "generate a short name for 'bar' (style)" );
    ...
    like(
        $@,
        qr/^dbit${v}_${ts}__$n \(provided by Data::ShortNameProvider::Style::Basic\) is longer than the $l characters limit /,
        '... expected error message'
    );
